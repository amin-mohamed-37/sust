...
/**
    ...
    // Fetch table names properly
    while ($row = mysqli_fetch_array($tables)) {
        $curr_table = $row[0]; // Get table name

        // Add table details to backup
        $sql_file .= "# --------------------------------------------------------\n";
    ...
    // Read the backup file into a string and remove the file
    if (file_exists($this->database_dump_filename)) {
        $finalbackup = file_get_contents($this->database_dump_filename);
        unlink($this->database_dump_filename);
    } else {
        $finalbackup = "Error: Backup file not found!";
    }
    ...
    foreach ($lines as $line) {
        ...
        // Execute query
        $result = mysqli_query($this->conn, $line);
        if (!$result) {
            $this->msg = "Error: " . mysqli_error($this->conn);
            return $this->msg;
        }
    }

    $this->msg = "Successfully restored the database.";
    return $this->msg;
	}
    ...
    if ($result) {
        if (mysqli_num_rows($result) > 0) {
            $sql_create_arr = mysqli_fetch_array($result, MYSQLI_NUM);
    ...
    for ($j = 0; $j < $fields_cnt; $j++) {
        $field_info = mysqli_fetch_field_direct($result, $j);
        $field_set[$j] = $this->sql_backquote($field_info->name);

        // If field is an integer type
        if (in_array($field_info->type, [MYSQLI_TYPE_TINY, MYSQLI_TYPE_SHORT, MYSQLI_TYPE_LONG, MYSQLI_TYPE_INT24, MYSQLI_TYPE_LONGLONG])) {
            $field_num[$j] = true;
        } else {
            $field_num[$j] = false;
        }
    ...
    while ($row = mysqli_fetch_row($result)) {
        ...
        for ($j = 0; $j < $fields_cnt; $j++) {
            if (!isset($row[$j])) {
                ...
                if ($field_num[$j]) {
                    $values[] = $row[$j]; // Numeric values remain unquoted
                } else {
